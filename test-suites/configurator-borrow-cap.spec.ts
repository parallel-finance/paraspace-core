...
makeSuite("PoolConfigurator: Borrow Cap", () => {
  let testEnv: TestEnv;
  ...
  it("Tries to borrow any variable DAI or USDC, (> BORROW_CAP) (revert expected)", async () => {
  ...
  it("Tries to borrows 100 variable DAI and 100 variable USDC (> BORROW_CAP) (revert expected)", async () => {
    const {usdc, pool, dai, deployer} = testEnv;
