...
describe("Leveraged Bid - Positive tests", () => {
  ...
  it("TC-erc721-bid-05 ERC20 <=> Punk accept use credit borrow and cash  offer", async () => {
    const {
      usdc,
      punk,
      wPunk,
    ...
    await waitForTx(
      await punk.connect(taker.signer)["getPunk(uint256)"](nftId)
    );
    expect(await punk.punkIndexToAddress(nftId)).to.be.equal(taker.address);
    await waitForTx(
      await punk.connect(taker.signer).offerPunkForSale(nftId, 0)
    );
    ...
    const tx = wPunkGateway.connect(taker.signer).acceptBidWithCredit(
    ...
    // maker nWPunk should increase
    expect(await nWPunk.balanceOf(maker.address)).to.be.equal(1);
  ...
  it("TC-erc721-bid-06 ERC20 <=> (ERC-721 & NToken) accept in batch", async () => {
    const {
      nBAYC,
      bayc,
      usdc,
  ...
  it("TC-erc721-bid-07 accept collection bid", async () => {
    const {
      mayc,
      nMAYC,
      usdc,
  ...
  it("TC-erc721-bid-08 accept collection set bid", async () => {
    const {
      mayc,
      nMAYC,
      usdc,
  ...
  it("TC-erc721-bid-09 accept with 2 platform fee item", async () => {
    const {
      mayc,
      usdc,
      conduitKey,
...
describe("Leveraged Bid - Negative tests", () => {
  ...
  it("TC-erc721-bid-10 collateral unable overwrite loand after accept offer (should fail)", async () => {
    const {
      nBAYC,
      dai,
      bayc,
  ...
  it("TC-erc721-bid-11 offer cannot be accept when liquidity is insufficient (should fail)", async () => {
    const {
      bayc,
      dai,
      pool,
  ...
  it("TC-erc721-bid-12 cannot purchase a non-matching NFT id (should fail)", async () => {
    const {
      bayc,
      dai,
      users: [maker, taker],
  ...
  it("TC-erc721-bid-13 cannot perform same purchase twice (should fail)", async () => {
    const {
      bayc,
      dai,
      users: [maker, taker],
  ...
  it("TC-erc721-bid-14 cannot with accept a not wrapped erc20 currency (should fail)", async () => {
    const {
      bayc,
      pDai,
      users: [maker, taker],
  ...
  it("TC-erc721-bid-15 cannot credit amount above the NFT's LTV (should fail)", async () => {
    const {
      bayc,
      dai,
      users: [maker, taker],
