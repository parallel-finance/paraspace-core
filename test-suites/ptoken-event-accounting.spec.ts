...
makeSuite("PToken Mint and Burn Event Accounting", (testEnv) => {
  ...
  const accruedDebt2: BigNumber = BigNumber.from(0);
  ...
  const xTokenMintEventSignature = utils.keccak256(
    utils.toUtf8Bytes("Mint(address,address,uint256,uint256,uint256)")
  );
  const xTokenBurnEventSignature = utils.keccak256(
  ...
  it("User 2 - borrow more Dai - confirm mint event includes accrued interest", async () => {
    ...
    accruedDebt1 = borrowerDaiData.currentVariableDebt
      .sub(firstDaiBorrow)
      .sub(secondDaiBorrow);
    ...
    expect(rawTransferEvents.length).to.equal(
      2,
      "Incorrect number of Transfer Events"
    );
    const parsedTransferEvent = variableDebtDai.interface.parseLog(
      rawTransferEvents[0]
    );
    ...
    const parsedMintEvent = variableDebtDai.interface.parseLog(
      rawMintEvents[0]
    );
  ...
  it("User 1 - deposit more Dai - confirm mint event includes accrued interest", async () => {
    ...
    const depositTx = await waitForTx(
      await pool
        .connect(depositor.signer)
        .supply(dai.address, secondDaiDeposit, depositor.address, "0")
    );

    const pDaiBalance = await pDai.balanceOf(depositor.address);
    accruedInterest1 = pDaiBalance.sub(firstDaiDeposit).sub(secondDaiDeposit);
    ...
    const parsedTransferEvent = pDai.interface.parseLog(rawTransferEvents[1]);

    // get mint event
    const rawMintEvents = depositTx.logs.filter(
      (log) => log.topics[0] === xTokenMintEventSignature
    );
    expect(rawMintEvents.length).to.equal(1, "Incorrect number of Mint Events");
    const parsedMintEvent = pDai.interface.parseLog(rawMintEvents[0]);
    ...
    expect(parsedMintEvent.args.value).to.be.closeTo(totalMinted, 2);
    expect(parsedMintEvent.args.balanceIncrease).to.be.closeTo(
      accruedInterest1,
      2
    );
  });
  ...
  it("User 1 - deposit more Dai again - confirm mint event includes accrued interest", async () => {
    ...
    const depositTx = await pool
      .connect(depositor.signer)
      .supply(dai.address, thirdDaiDeposit, depositor.address, "0");
    const depositReceipt = await depositTx.wait();

    const pDaiBalance = await pDai.balanceOf(depositor.address);
    accruedInterest2 = pDaiBalance
      .sub(firstDaiDeposit)
  ...
  it("User 2 - repay all remaining dai", async () => {
    ...
    const parsedBurnEvent = variableDebtDai.interface.parseLog(
      rawBurnEvents[0]
    );
  ...
  it("User 1 - withdraws all deposited funds and interest", async () => {
    ...
    const pDaiBalance = await pDai.balanceOf(depositor.address);
    expect(pDaiBalance).to.be.equal(0);
    ...
    const parsedTransferEvent = pDai.interface.parseLog(rawTransferEvents[0]);

    // get burn event
    const rawBurnEvents = withdrawReceipt.logs.filter(
      (log) => log.topics[0] === xTokenBurnEventSignature
    );
    expect(rawBurnEvents.length).to.equal(1, "Incorrect number of Burn Events");
    const parsedBurnEvent = pDai.interface.parseLog(rawBurnEvents[0]);
    ...
    expect(parsedBurnEvent.args.value).to.be.closeTo(totalBurned, 2);
    expect(parsedBurnEvent.args.balanceIncrease).to.be.closeTo(
      accruedInterest3,
      2
    );
  ...
  it("User 1 - withdraw amount less than accrued interest", async () => {
    ...
    expect(parsedMintEvent.args.value).to.be.closeTo(totalMinted, 2);
    expect(parsedMintEvent.args.balanceIncrease).to.be.closeTo(
      totalMinted.add(smallWithdrawal),
      2
    );
    expect(parsedMintEvent.args.index).to.equal(daiReserveData.liquidityIndex);
  });
