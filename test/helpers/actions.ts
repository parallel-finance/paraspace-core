  ...
  if (expectedResult === "success") {
    const txResult = await waitForTx(
      await pool
        ...
        .supply(reserve, amountToDeposit, onBehalfOf, "0", txOptions)
    );
...
export const delegateBorrowAllowance = async (
  ...
  const reserveAddress: tEthereumAddress = await getTokenAddressFromSymbol(
    reserve
  );
...
export const borrow = async (
  ...
  if (expectedResult === "success") {
    ...
    if (timeTravel) {
      const secondsToTravel = BigNumber.from(timeTravel)
        .mul(ONE_YEAR)
        .div(365)
        .toNumber();
...
const expectEqual = (
  ...
  if (!configuration.skipIntegrityCheck) {
    // eslint-disable-next-line
    (expect(actual).to.be as any).almostEqualOrEqual(expected);
  }
};
...
const getDataBeforeAction = async (
  ...
  const xTokenInstance = await getPToken(reserveData.xTokenAddress);
  return {
    reserve,
    reserveData,
    userData,
...
export const getContractsData = async (
  ...
  const {pool, protocolDataProvider} = testEnv;

  const [userData, reserveData, timestamp] = await Promise.all([
    getUserData(pool, protocolDataProvider, reserve, user, sender || user),
    getReserveData(protocolDataProvider, reserve),
    timeLatest(),
  ]);
