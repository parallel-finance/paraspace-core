...
import {LiquidityManager} from "../types";

describe("IZUMI NFT Oracle", () => {
  let snap: string;
  let testEnv: TestEnv;
  ...
  it("test with dai and weth:(token0 decimal equals token1 decimal) [ @skip-on-coverage ]", async () => {
    ...
    let tokenPrice = await izumiOracle.getTokenPrice(tokenId);
    almostEqual(
      tokenPrice,
      liquidityWethAmount.add(liquidityDaiAmount.div(1000))
    );

    const izumiPool = await getIZUMIPool({token0: dai, token1: weth, fee});
    ...
    let currentPrice = (await izumiPool.state()).sqrtPrice_96;
    expect(currentPrice).to.gt(lowerPrice.toString());
    expect(currentPrice).to.lt(upperPrice.toString());

    const targetDaiFee = await convertToCurrencyDecimals(dai.address, "2");
    const targetEthFee = await convertToCurrencyDecimals(weth.address, "0.002");
    const targetTokenPrice = liquidityWethAmount
      .add(targetEthFee)
      .add(liquidityDaiAmount.div(1000))
      .add(targetDaiFee.div(1000));
    lpFee = await izumiOracle.getLpFeeAmount(tokenId);
    almostEqual(lpFee.token0Amount, targetDaiFee);
    almostEqual(lpFee.token1Amount, targetEthFee);
    tokenPrice = await izumiOracle.getTokenPrice(tokenId);
    ...
    currentPrice = (await izumiPool.state()).sqrtPrice_96;
    expect(currentPrice).to.lt(lowerPrice.toString());

    const addDaiFeeCap = await convertToCurrencyDecimals(dai.address, "200");
    lpFee = await izumiOracle.getLpFeeAmount(tokenId);
    ...
    const addEthFeeCap = await convertToCurrencyDecimals(weth.address, "20");
    lpFee = await izumiOracle.getLpFeeAmount(tokenId);
  ...
  it("test with usdc and weth:(token0 decimal less than token1 decimal) [ @skip-on-coverage ]", async () => {
    ...
    const izumiPool = await getIZUMIPool({token0: usdc, token1: weth, fee});
    //check lp fee in range
    ...
    const targetUsdcFee = await convertToCurrencyDecimals(usdc.address, "2");
    const targetEthFee = await convertToCurrencyDecimals(weth.address, "0.002");
    const targetTokenPrice = liquidityWethAmount
      .add(targetEthFee)
    ...
    const addUsdcFeeCap = await convertToCurrencyDecimals(usdc.address, "200");
    lpFee = await izumiOracle.getLpFeeAmount(tokenId);
  ...
  it("test with weth and usdt:(token0 decimal greater than token1 decimal) [ @skip-on-coverage ]", async () => {
    ...
    const izumiPool = await getIZUMIPool({token0: usdt, token1: weth, fee});
    //check lp fee in range
    ...
    const targetUsdtFee = await convertToCurrencyDecimals(usdt.address, "2");
    const targetEthFee = await convertToCurrencyDecimals(weth.address, "0.002");
    const targetTokenPrice = liquidityWethAmount
      .add(targetEthFee)
    ...
    const addUsdtFeeCap = await convertToCurrencyDecimals(usdt.address, "200");
    lpFee = await izumiOracle.getLpFeeAmount(tokenId);
    almostEqual(lpFee.token0Amount, targetEthFee);
