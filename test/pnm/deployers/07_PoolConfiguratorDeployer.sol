...
contract PoolConfiguratorDeployer is Deployer {
    ...
    function deploy() public override FromDeployer {
        address proivderAddr = config.contractAddresses("PoolAddressesProvider");
        ...
        PoolConfigurator configuratorProxy = PoolConfigurator(provider.getPoolConfigurator());
        configuratorProxy.setAuctionRecoveryHealthFactor(config.auctionRecoveryHealthFactor());
    }
}
