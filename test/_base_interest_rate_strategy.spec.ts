...
describe("Interest Rate Tests", () => {
  context("InterestRateStrategy", () => {
    ...
    it("TC-interest-rate-strategy-01 Checks rates at 0% usage ratio, empty reserve", async () => {
      ...
      const {0: stableBorrowRate, 1: variableBorrowRate} =
        await strategyInstance.calculateBorrowRates(0, 0, 0);
      const liquidityRate = await strategyInstance.calculateLiquidityRate(
        0,
        0,
        0,
        variableBorrowRate,
      ...
      expect(currentLiquidityRate).to.be.equal(liquidityRate);
      expect(currentStableBorrowRate).to.be.equal(stableBorrowRate);
      expect(currentVariableBorrowRate).to.be.equal(variableBorrowRate);
    });
    ...
    it("TC-interest-rate-strategy-02 Checks rates at 80% usage ratio", async () => {
      ...
      const {0: stableBorrowRate, 1: variableBorrowRate} =
        await strategyInstance.calculateBorrowRates(
          "200000000000000000",
          0,
          "800000000000000000"
        );
