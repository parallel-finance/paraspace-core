...
const boost_JSON = {
  ...
  Azuki: {
    ...
    Golden_Red_Panda_Kigurumi: {
      boost: 2.3,
      ids: ["3945", "777", "847", "9211", "1693"],
    },
    Golden_Sloth_Kigurumi: {
      boost: 3.4,
      ids: ["5167", "1108", "6282", "7514", "120", "1578"],
    },
    Golden_Bean: {
      boost: 1.2,
      ids: [
    ...
    Golden_Zanbato: {
      boost: 2.0,
      ids: ["2540", "598", "7821", "5208", "6137", "930", "683", "5686"],
    },
    Golden_Bat: {
      boost: 1.3,
      ids: ["7488", "1637", "4761", "5028", "2076", "4667", "4791", "472"],
    },
    Golden_Sheathed_Katana: {
      boost: 1.2,
      ids: ["2575", "3291", "520", "4684", "6932", "6531", "2108"],
    },
  },
  Otherdeeds: {
    Otherdeeds_Koda: {
...
// const BAYCTraits = Object.values(boost_JSON["BAYC"]);
//
// const MAYCTraits = Object.values(boost_JSON["MAYC"]);
//
// const BAKCTraits = Object.values(boost_JSON["BAKC"]);
//
// const OTHRTraits = Object.values(boost_JSON["Otherdeeds"]);
//
const AZUKITraits = Object.values(boost_JSON["Azuki"]);

const BEANZTraits = Object.values(boost_JSON["BEANZ"]);
const WPUNKSTraits = Object.values(boost_JSON["WPUNKS"]);
...
const setTraitsMultipliers = async () => {
  ...
  const wpunks = allTokens[ERC721TokenContractId.WPUNKS];
  const beanz = allTokens[ERC721TokenContractId.BEANZ];
  ...
  const nBEANZ = await getNToken(
    (
      await pool.getReserveData(beanz.address)
    ).xTokenAddress
  );
  const nWPUNKS = await getNToken(
    (
      await pool.getReserveData(wpunks.address)
    ).xTokenAddress
  );
  ...
  ////////////////////////////////////////////////////////////////////////////////
  if (DRY_RUN) {
    const tokenIds = BEANZTraits.map((x) => x.ids).flat();
    const multipliers = BEANZTraits.map((x) =>
      x.ids.map(() => toBn(x.boost.toString()))
    ).flat();
    ...
    for (const [tokenIds, multipliers] of chunked) {
      const encodedData = nBEANZ.interface.encodeFunctionData(
        "setTraitsMultipliers",
        [tokenIds!, multipliers!]
      );
      await dryRunEncodedData(nBEANZ.address, encodedData);
    }
  }
  ...
  if (DRY_RUN) {
    const tokenIds = WPUNKSTraits.map((x) => x.ids).flat();
    const multipliers = WPUNKSTraits.map((x) =>
      x.ids.map(() => toBn(x.boost.toString()))
    ).flat();
    ...
    for (const [tokenIds, multipliers] of chunked) {
      const encodedData = nWPUNKS.interface.encodeFunctionData(
        "setTraitsMultipliers",
        [tokenIds!, multipliers!]
      );
      await dryRunEncodedData(nWPUNKS.address, encodedData);
    }
  }
