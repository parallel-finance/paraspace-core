...
const filter_zksync_user = async () => {
  ...
  if (fs.existsSync(ZK_USERS_PATH)) {
    ...
    const valueUserArr0: Array<string> = JSON.parse(
      fs.readFileSync(ZK_USERS_LEVEL_0_PATH, "utf8")
    );
    const valueUserArr1: Array<string> = JSON.parse(
      fs.readFileSync(ZK_USERS_LEVEL_1_PATH, "utf8")
    );
    const valueUserArr2: Array<string> = JSON.parse(
      fs.readFileSync(ZK_USERS_LEVEL_2_PATH, "utf8")
    );
