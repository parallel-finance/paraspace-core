...
library LiquidationLogic {
     ...
     * @dev Emits the `executeERC721LiquidationWithCollateralSwapCall()` event
     * @param reservesData The state of all the reserves
     * @param reservesList The addresses of all the active reserves
     * @param usersConfig The users configuration mapping that track the supplied/borrowed assets
     * @param params The additional parameters needed to execute the liquidation function
    ...
    function executeERC721LiquidationWithEther(
        ...
        if (vars.actualDebtToLiquidate > availableLiquidationAmount) {
        ...
        SupplyLogic.executeSupply(
            ...
            DataTypes.ExecuteSupplyParams({
                asset: params.liquidationAsset,
                amount: availableLiquidationAmount - vars.actualDebtToLiquidate,
                onBehalfOf: params.user,
                referralCode: 0
            })
        );

        if (!userConfig.isUsingAsCollateral(liquidationAssetReserveId)) {
            userConfig.setUsingAsCollateral(liquidationAssetReserveId, true);
            emit ReserveUsedAsCollateralEnabled(
                params.liquidationAsset,
                params.user
            );
