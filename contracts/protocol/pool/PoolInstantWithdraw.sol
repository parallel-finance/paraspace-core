    ...
    IPoolInstantWithdraw
{
    ...
    function createLoan(
        ...
        uint256 nextBorrowRate = 0;
        // calculate stable borrow rate
        {
            uint256 availableLiquidity = IERC20(borrowAsset).balanceOf(
                reserveCache.xTokenAddress
            ) - borrowAmount;
            uint256 totalStableDebt = reserveCache.nextTotalStableDebt +
                borrowAmount;
            uint256 totalVariableDebt = reserveCache
                .nextScaledVariableDebt
                .rayMul(reserveCache.nextVariableBorrowIndex);

            (nextBorrowRate, ) = IReserveInterestRateStrategy(
                reserve.interestRateStrategyAddress
            ).calculateBorrowRates(
                    availableLiquidity,
                    totalStableDebt,
                    totalVariableDebt
                );
