...
contract DefaultReserveInterestRateStrategy is IReserveInterestRateStrategy {
        ...
        )
    {
        ...
        ) = _calculateBorrowRates(
                availableLiquidity,
                totalStableDebt,
                totalVariableDebt
            );

        uint256 liquidityRate = _calculateLiquidityRates(
            totalStableDebt,
            totalVariableDebt,
            variableBorrowRate,
    ...
    function calculateBorrowRates(
        ...
        view
        override
        returns (uint256 stableBorrowRate, uint256 variableBorrowRate)
    {
        (, stableBorrowRate, variableBorrowRate) = _calculateBorrowRates(
            availableLiquidity,
            totalStableDebt,
            totalVariableDebt
        );
    ...
    function _calculateBorrowRates(
        uint256 availableLiquidity,
        uint256 totalStableDebt,
        uint256 totalVariableDebt
    )
        ...
        )
    {
        ...
        if (vars.totalDebt != 0) {
            vars.stableToTotalDebtRatio = totalStableDebt.rayDiv(
                vars.totalDebt
            );
    ...
    function _calculateLiquidityRates(
        uint256 totalStableDebt,
        uint256 totalVariableDebt,
        uint256 currentVariableBorrowRate,
        uint256 currentAverageStableBorrowRate,
        ...
        uint256 currentLiquidityRate = overallBorrowRate
            .rayMul(usageRatio)
