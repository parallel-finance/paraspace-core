...
contract LendPoolConfigurator is Initializable, ILendPoolConfigurator {
  ...
  function setBorrowingFlagOnReserve(address[] calldata assets, bool flag) external onlyPoolAdmin {
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      BDaoDataTypes.ReserveConfigurationMap memory currentConfig = cachedPool.getReserveConfiguration(assets[i]);
  ...
  function batchConfigReserve(ConfigReserveInput[] calldata inputs) external onlyPoolAdmin {
    ILendPool cachedPool = _getLendPool();
    for (uint256 i = 0; i < inputs.length; i++) {
      BDaoDataTypes.ReserveConfigurationMap memory currentConfig = cachedPool.getReserveConfiguration(inputs[i].asset);
  ...
  function setActiveFlagOnNft(address[] calldata assets, bool flag) external onlyPoolAdmin {
    ILendPool cachedPool = _getLendPool();
    for (uint256 i = 0; i < assets.length; i++) {
      BDaoDataTypes.NftConfigurationMap memory currentConfig = cachedPool.getNftConfiguration(assets[i]);
  ...
  function batchConfigNft(ConfigNftInput[] calldata inputs) external onlyPoolAdmin {
    ILendPool cachedPool = _getLendPool();
    for (uint256 i = 0; i < inputs.length; i++) {
      BDaoDataTypes.NftConfigurationMap memory currentConfig = cachedPool.getNftConfiguration(inputs[i].asset);
  ...
  function _checkReserveNoLiquidity(address asset) internal view {
    BDaoDataTypes.ReserveData memory reserveData = _getLendPool().getReserveData(asset);
