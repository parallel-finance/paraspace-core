...
contract LendPoolLoan is Initializable, ILendPoolLoan, ContextUpgradeable, IERC721ReceiverUpgradeable {
  ...
  mapping(uint256 => BDaoDataTypes.LoanData) private _loans;
  ...
  function createLoan(
    ...
    // Save Info
    BDaoDataTypes.LoanData storage loanData = _loans[loanId];
  ...
  function updateLoan(
    ...
    BDaoDataTypes.LoanData storage loan = _loans[loanId];

    // Ensure valid loan state
    require(loan.state == BDaoDataTypes.LoanState.Active, Errors.LPL_INVALID_LOAN_STATE);
  ...
  function repayLoan(
    ...
    _loans[loanId].state = BDaoDataTypes.LoanState.Repaid;
  ...
  function auctionLoan(
    ...
    if (loan.bidStartTimestamp == 0) {
      ...
      loan.state = BDaoDataTypes.LoanState.Auction;
      loan.bidStartTimestamp = block.timestamp;
      loan.firstBidderAddress = onBehalfOf;
    } else {
      require(loan.state == BDaoDataTypes.LoanState.Auction, Errors.LPL_INVALID_LOAN_STATE);
  ...
  function redeemLoan(
    ...
    loan.state = BDaoDataTypes.LoanState.Active;
  ...
  function liquidateLoan(
    ...
    // state changes and cleanup
    // NOTE: these must be performed before assets are released to prevent reentrance
    _loans[loanId].state = BDaoDataTypes.LoanState.Defaulted;
  ...
  function getLoan(uint256 loanId) external view override returns (BDaoDataTypes.LoanData memory loanData) {
