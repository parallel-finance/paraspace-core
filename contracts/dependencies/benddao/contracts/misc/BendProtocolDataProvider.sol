...
contract BendProtocolDataProvider {
  ...
  function getAllReservesTokenDatas() external view returns (ReserveTokenData[] memory) {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      BDaoDataTypes.ReserveData memory reserveData = pool.getReserveData(reserves[i]);
  ...
  function getReserveTokenData(address asset) external view returns (ReserveTokenData memory) {
    ILendPool pool = ILendPool(ADDRESSES_PROVIDER.getLendPool());
    BDaoDataTypes.ReserveData memory reserveData = pool.getReserveData(asset);
    return
      ReserveTokenData({
  ...
  function getAllNftsTokenDatas() external view returns (NftTokenData[] memory) {
    ...
    for (uint256 i = 0; i < nfts.length; i++) {
      BDaoDataTypes.NftData memory nftData = pool.getNftData(nfts[i]);
  ...
  function getNftTokenData(address nftAsset) external view returns (NftTokenData memory) {
    ILendPool pool = ILendPool(ADDRESSES_PROVIDER.getLendPool());
    BDaoDataTypes.NftData memory nftData = pool.getNftData(nftAsset);
    return
      NftTokenData({
  ...
  {
    BDaoDataTypes.ReserveConfigurationMap memory configuration = ILendPool(ADDRESSES_PROVIDER.getLendPool())
  ...
  function getNftConfigurationData(address asset) external view returns (NftConfigurationData memory configData) {
    BDaoDataTypes.NftConfigurationMap memory configuration = ILendPool(ADDRESSES_PROVIDER.getLendPool())
  ...
  {
    BDaoDataTypes.ReserveData memory reserve = ILendPool(ADDRESSES_PROVIDER.getLendPool()).getReserveData(asset);
    ...
    returns (LoanData memory loanData)
  {
    ...
    BDaoDataTypes.LoanData memory loan = ILendPoolLoan(ADDRESSES_PROVIDER.getLendPoolLoan()).getLoan(loanData.loanId);
    _fillLoanData(loanData, loan);
  }

  function getLoanDataByLoanId(uint256 loanId) external view returns (LoanData memory loanData) {
    BDaoDataTypes.LoanData memory loan = ILendPoolLoan(ADDRESSES_PROVIDER.getLendPoolLoan()).getLoan(loanId);
    _fillLoanData(loanData, loan);
  }

  function _fillLoanData(LoanData memory loanData, BDaoDataTypes.LoanData memory loan) internal view {
