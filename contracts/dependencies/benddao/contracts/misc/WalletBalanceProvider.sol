...
contract WalletBalanceProvider {
    ...
    returns (address[] memory, uint256[] memory)
  {
    ...
    for (uint256 j = 0; j < reserves.length; j++) {
      BDaoDataTypes.ReserveConfigurationMap memory configuration = pool.getReserveConfiguration(reservesWithEth[j]);
    ...
    returns (address[] memory, uint256[] memory)
  {
    ...
    for (uint256 j = 0; j < nfts.length; j++) {
      /*
      BDaoDataTypes.NftConfigurationMap memory configuration = pool.getNftConfiguration(nfts[j]);
